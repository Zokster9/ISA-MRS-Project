            ...
            <div class="d-flex flex-column" style="width: 50%">
                <div class="d-flex" style="margin: 10px">
                    <div style="width: 100%; border: 1px solid #323539;">
                        <img :src="require(`../assets/${retreat.pictures[0]}`)" width="100%">
                    </div> 
                </div>
                <div class="d-flex flex-row">
                    <template v-for="(picture, index) in retreat.pictures">
                        <div style="margin: 10px" :key="index">
                            <img :id="picture" :src="require(`../assets/${picture}`)" style="width: 89px; height: 89px">
                        </div>
                    </template>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        mounted(){
            axios.get("http://localhost:8088/retreats/" + this.$route.params.id).then((response) => {
